...
class Pipeline:
    def __init__(self, config: Config, data_generator: DataGenerator, task_creator: TaskCreator, llm_provider: LLMProvider):
        self.config = config
    ...
    async def _generate_data_points(self):
        for _ in range(self.config.get('num_samples')):
            try:
